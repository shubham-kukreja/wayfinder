              ...
              <div className="divide-y divide-neutral-100 border-t border-neutral-100">
                {items.map(({ key, state, impact }) => {
                  ...
                  return (
                    <div key={key}>
                      <div
                        className={`flex items-center justify-between gap-4 px-4 py-2.5 text-sm ${rubricSpec ? "cursor-pointer hover:bg-neutral-50" : ""}`}
                        onClick={rubricSpec ? () => setExpandedRubricKey(isRubricExpanded ? null : key) : undefined}
                      >
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-neutral-800">
                            {key}
                            {rubricSpec && <span className="ml-1 text-neutral-400">{isRubricExpanded ? "▾" : "▸"}</span>}
                          </div>
                          {state.note && <div className="truncate text-xs text-neutral-400">{state.note}</div>}
                          {provenance === "default" && (
                            <div className="text-xs text-neutral-400">No fetched or manual input — contributing nothing to the allocation</div>
                          )}
                          {rubricSpec && !isRubricExpanded && <div className="text-xs text-neutral-400">Click to pick conditions</div>}
                        </div>
